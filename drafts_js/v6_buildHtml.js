        ...
        if (data[1] instanceof Object) {
           if (typeof data[2] != 'string') {
               ...
               // tagAttrKey = Object.keys(data[1]);
                 tagKeyVal= Object.keys(tagAttr).reduce((acc, value) => acc += ` ${value}="${tagAttr[value]}"`, '');

                // return `<${data[0]} ${tagAttrKey}=${tagAttr[tagAttrKey]}></${data[0]}>`;
        ...
        if (data[1] instanceof Object) {
           if (typeof data[2] != 'string') {
               ...
               // tagAttrKey = Object.keys(data[1]);
                ...
                return `<${data[0]} ${tagKeyVal}>${data[2].map(buildHtml).join('')}</${data[0]}>`;
           }

            tagAttr = data[1];
            tagKeyVal= Object.keys(tagAttr).reduce((acc, value) => acc += ` ${value}="${tagAttr[value]}"`, '');

            return `<${data[0]}${tagKeyVal}>${data[2]}</${data[0]}>`;
        }
    }
